...
// ListConversationMembers handles GET /api/conversations/{conversation_id}/members.
func (h *Handler) ListConversationMembers(w http.ResponseWriter, r *http.Request) {
	if h.service == nil || h.sessions == nil {
		writeJSON(w, http.StatusServiceUnavailable, Response{Success: false, Code: "SERVICE_UNAVAILABLE", Message: "服务尚未完成初始化"})
		return
	}
	...
	resp, status, svcErr := h.service.ListConversationMembers(actor, conversationID)
	if svcErr != nil {
		writeJSON(w, status, errorResponse(svcErr))
		return
	}
...
// UpdateConversationNotificationMute handles PATCH /api/conversations/{conversation_id}/notification-mute.
func (h *Handler) UpdateConversationNotificationMute(w http.ResponseWriter, r *http.Request) {
	if h.service == nil || h.sessions == nil {
		writeJSON(w, http.StatusServiceUnavailable, Response{Success: false, Code: "SERVICE_UNAVAILABLE", Message: "服务尚未完成初始化"})
		return
	}
	...
	var req UpdateConversationNotificationMuteRequest
	if err := decodeJSON(r, &req); err != nil {
		writeJSON(w, http.StatusBadRequest, Response{Success: false, Code: "INVALID_REQUEST", Message: "请求格式错误"})
		return
	}

	resp, status, svcErr := h.service.UpdateConversationNotificationMute(actor, conversationID, req)
	if svcErr != nil {
		writeJSON(w, status, errorResponse(svcErr))
		return
	}
...
// UpdateConversationAutoDelete handles PATCH /api/conversations/{conversation_id}/auto-delete.
func (h *Handler) UpdateConversationAutoDelete(w http.ResponseWriter, r *http.Request) {
	if h.service == nil || h.sessions == nil {
		writeJSON(w, http.StatusServiceUnavailable, Response{Success: false, Code: "SERVICE_UNAVAILABLE", Message: "服务尚未完成初始化"})
		return
	}
	...
	var req UpdateConversationAutoDeleteRequest
	if err := decodeJSON(r, &req); err != nil {
		writeJSON(w, http.StatusBadRequest, Response{Success: false, Code: "INVALID_REQUEST", Message: "请求格式错误"})
		return
	}

	resp, status, svcErr := h.service.UpdateConversationAutoDelete(actor, conversationID, req)
	if svcErr != nil {
		writeJSON(w, status, errorResponse(svcErr))
		return
	}
...
func (h *Handler) AddConversationMembers(w http.ResponseWriter, r *http.Request) {
	...
	var req AddConversationMembersRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeJSON(w, http.StatusBadRequest, Response{Success: false, Code: "INVALID_REQUEST", Message: "JSON 格式错误"})
		return
	}

	resp, status, svcErr := h.service.AddConversationMembers(actor, conversationID, req)
	if svcErr != nil {
		writeJSON(w, status, errorResponse(svcErr))
		return
	}
...
// RemoveConversationMember handles DELETE /api/conversations/{conversation_id}/members.
func (h *Handler) RemoveConversationMember(w http.ResponseWriter, r *http.Request) {
	...
	resp, status, svcErr := h.service.RemoveConversationMember(actor, conversationID, req)
	if svcErr != nil {
		writeJSON(w, status, errorResponse(svcErr))
		return
	}
